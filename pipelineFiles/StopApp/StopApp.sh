...
cd ${domainWlBirc} && nohup ./startNodeManager.sh > nodemanager.log 2>&1 &
cd ${domainWlBirc} && . ./setDomainEnv.sh ENV && java weblogic.Deployer -adminurl ${urlWlBirc} -username ${userwlBirc} -password ${passwlBirc} -stop -name ${artifactNameWlBirc};